-- =====================================================
-- FIX GET_MECHANIC_LEADS RPC - PENDING → SEARCHING
-- =====================================================
-- Purpose: Fix the get_mechanic_leads RPC to use 'searching' instead of 'pending'
-- Issue: RPC was checking for 'pending' status which doesn't exist in the enum
-- =====================================================

-- Drop and recreate the get_mechanic_leads function with correct status values
...
-- Drop and recreate the summary function
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE j.status = 'searching' AND NOT EXISTS(SELECT 1 FROM quote_requests qr WHERE qr.job_id = j.id AND qr.mechanic_id = p_mechanic_id))::integer AS new_leads,
    COUNT(*) FILTER (WHERE EXISTS(SELECT 1 FROM quote_requests qr WHERE qr.job_id = j.id AND qr.mechanic_id = p_mechanic_id))::integer AS quoted_leads
  FROM jobs j
  INNER JOIN profiles p ON p.auth_id = j.customer_id
  WHERE
    j.deleted_at IS NULL
