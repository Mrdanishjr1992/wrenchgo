-- ============================================================================
-- Migration: 20250213000006_fix_job_status_in_leads.sql
-- ============================================================================
-- Purpose: Remove invalid 'open' status from get_mechanic_leads filter
-- Dependencies: 20250213000005 (get_mechanic_leads function)
-- Risk Level: Low (function replacement, fixes bug)
-- Rollback: Restore previous get_mechanic_leads function
-- ============================================================================

-- Fix get_mechanic_leads to use correct job_status enum values
-- Remove 'open' which doesn't exist in the enum
  ...
  WHERE
    ...
    AND (
      CASE p_filter
        ...
        ELSE j.status::text IN ('searching', 'quoted', 'draft')
      END
    )
  ORDER BY
