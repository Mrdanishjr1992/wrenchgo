-- Migration: Update get_mechanic_leads RPC to use public location fields
-- Purpose: Ensure mechanics only see approximate location until job acceptance

-- 1. Update get_mechanic_leads to use public location fields
...
-- 2. Update get_mechanic_leads_summary to use public location fields
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (
      WHERE j.public_latitude IS NOT NULL
        AND j.public_longitude IS NOT NULL
        AND v_mechanic_lat IS NOT NULL
        AND v_mechanic_lng IS NOT NULL
        AND public.calculate_distance_miles(
    ...
    COUNT(*) FILTER (WHERE jq.job_id IS NOT NULL)::INTEGER AS quoted_count
  FROM jobs j
  LEFT JOIN job_quotes jq ON j.id = jq.job_id
  WHERE
    j.status = 'searching'
...
COMMENT ON FUNCTION public.get_mechanic_leads IS 'Returns job leads for mechanics using PUBLIC location fields only';
COMMENT ON FUNCTION public.get_mechanic_leads_summary IS 'Returns lead counts using PUBLIC location fields only';
