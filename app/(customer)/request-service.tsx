...
export default function RequestService() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Persist service area / hub linkage (best-effort)
      // This helps ensure profiles.hub_id + waitlist are kept up to date.
      if (locationLat != null && locationLng != null) {
        let zip: string | null = null;
        const trimmed = location.trim();

        // If user typed a ZIP directly, use it.
        if (/^\d{5}$/.test(trimmed)) {
          zip = trimmed;
        } else {
          // Otherwise try reverse geocoding to get the postal code.
          try {
            const rev = await Location.reverseGeocodeAsync({
              latitude: locationLat,
              longitude: locationLng,
            });
            zip = rev?.[0]?.postalCode ?? null;
          } catch {
        ...
        try {
          for (const photo of pendingPhotos) {
            ...
            await supabase.rpc("create_job_media_record", {
              ...
              p_taken_lat: locationLat ?? null,
              p_taken_lng: locationLng ?? null,
            });
          }
        } catch (photoError) {
