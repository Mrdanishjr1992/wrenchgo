...
export default function AdminJobDetailScreen() {
  ...
  const canCapture = Boolean(
    contract?.id &&
      contract.payment_authorized_at &&
      !contract.payment_captured_at
  );

  const captureEnabled =
    canCapture &&
    (job.status === 'completed' || job.status === 'complete' || !!job.completed_at);
  ...
  const handleCapturePayment = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('capture-contract-payment', {
      ...
      setCaptureOk(data?.message ?? 'Payment captured');

      // Refresh detail so UI reflects captured_at/status changes.
      const updated = await adminGetJobDetail(id);
      setDetail(updated);
    } catch (err: any) {
      console.error('Capture error:', err);
      setCaptureError(err?.message || 'Failed to capture payment');
    } finally {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background, paddingTop: insets.top, paddingBottom: insets.bottom }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: spacing.lg }}>
        ...
        {/* Escrow / Payment */}
        {contract && (
          <Section title="Escrow / Payment (Stripe)">
            <View style={{ backgroundColor: colors.surface, borderRadius: 8, padding: spacing.md }}>
              <InfoRow label="Contract Status" value={contract.status} />
              <InfoRow label="Payment Status" value={contract.payment_status ?? "—"} accent />
              <InfoRow
                label="Authorized"
                value={contract.payment_authorized_at ? formatDateTime(contract.payment_authorized_at) : "Not yet"}
              />
              <InfoRow
                label="Captured"
                value={contract.payment_captured_at ? formatDateTime(contract.payment_captured_at) : "Not yet"}
              />
              {contract.stripe_payment_intent_id && (
                <InfoRow label="Payment Intent" value={contract.stripe_payment_intent_id} />
              )}
              ...
              {canCapture && (
                <>
                  ...
                  <TouchableOpacity
                    ...
                    disabled={!captureEnabled || captureLoading}
                    style={{
                      backgroundColor: captureEnabled ? colors.accent : colors.border,
                      borderRadius: 8,
                      paddingVertical: 10,
                      alignItems: "center",
                  ...
                  >
                    {captureLoading ? (
                      ...
                      <Text style={{ color: captureEnabled ? "#FFFFFF" : colors.textMuted, fontWeight: "700" }}>
                        Capture Payment Now
                      </Text>
                    )}
                  </TouchableOpacity>
                  ...
                  {!captureEnabled && (
                    <Text style={{ color: colors.textMuted, fontSize: 11, marginTop: spacing.sm }}>
                      Capture becomes available once the job is completed.
                    </Text>
                  )}
                </>
              )}
              ...
              {captureOk && (
                <Text style={{ color: "#10B981", fontSize: 12, marginTop: spacing.sm }}>{captureOk}</Text>
              )}
              {captureError && (
                <Text style={{ color: "#EF4444", fontSize: 12, marginTop: spacing.sm }}>{captureError}</Text>
              )}
            </View>
          </Section>
        )}
