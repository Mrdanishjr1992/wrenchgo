-- STANDALONE SYMPTOM SETUP
-- Run this directly in Supabase SQL Editor
-- Safe to run multiple times

-- Step 1: Create symptom_mappings table
...
-- Step 4: Create RLS policy (drop first if exists)
DROP POLICY IF EXISTS "symptom_mappings_public_read" ON public.symptom_mappings;
CREATE POLICY "symptom_mappings_public_read"
  ON public.symptom_mappings
  FOR SELECT
  TO anon, authenticated
  USING (true);

-- Step 5: Insert Engine symptoms
INSERT INTO public.symptom_mappings (symptom_key, symptom_label, category, risk_level, quote_strategy, customer_explainer) VALUES
...
-- Step 6: Create symptom_questions table
...
-- Step 7: Create indexes for symptom_questions
...
-- Step 8: Enable RLS for symptom_questions
ALTER TABLE public.symptom_questions ENABLE ROW LEVEL SECURITY;

-- Step 9: Create RLS policy for symptom_questions
DROP POLICY IF EXISTS "symptom_questions_public_read" ON public.symptom_questions;
CREATE POLICY "symptom_questions_public_read"
  ON public.symptom_questions
  FOR SELECT
  TO anon, authenticated
  USING (true);

-- Step 10: Verify - show all engine symptoms
SELECT 
  symptom_key, 
  symptom_label, 
  category, 
