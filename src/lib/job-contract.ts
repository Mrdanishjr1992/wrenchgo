...
export async function acceptQuoteAndCreateContract(
  ...
  // IMPORTANT:
  // Prefer the conflict-aware RPC (added in the DB migration that prevents double booking).
  // This keeps mechanic calendars clean and avoids overlapping contracts.
  const { data, error } = await supabase.rpc('accept_quote_and_resolve_conflicts', {
    p_quote_id: quoteId,
