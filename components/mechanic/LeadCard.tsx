...
export function LeadCard({ lead, onPressView, onPressQuote }: LeadCardProps) {
  ...
  const parseJobDescription = (): { symptom: string; details: string[] } => {
    try {
      const parsed = JSON.parse(lead.description);
      const symptom = parsed.symptom?.label || 'Service Request';
      ...
      return { symptom, details: details.slice(0, 3) };
    } catch {
      return { symptom: lead.description || 'Service Request', details: [] };
    }
  };

  const vehicleDisplay = getVehicleDisplay();
  const { symptom, details } = parseJobDescription();
