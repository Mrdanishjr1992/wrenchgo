...
export function JobActions({
  ...
  const handleAction = async (
    action: () => Promise<{ success: boolean; error?: string; contract_status?: string }>,
    ...
    try {
      ...
      if (!result.success) {
        ...
        // If this action caused the contract to become "completed", attempt to capture the
        // escrowed PaymentIntent as a best-effort convenience. (The edge function is idempotent.)
        if (result.contract_status === 'completed' && contract?.id) {
          try {
            await supabase.functions.invoke('capture-contract-payment', {
              body: { contract_id: contract.id },
            });
          } catch {
            // Best-effort only; capture might be handled by an admin workflow or retried later.
          }
        }
        if (successMessage) {
